		...
		<div class="layui-form-item layui-form-text" id="apply1">
			...
			<div class="layui-input-block">
				<textarea id="tReason1" placeholder="输入具体任务内容" class="layui-textarea"
					style="height: 300px">${task.TDirectorview}</textarea>
			</div>
		</div>
		<div class="layui-form-item layui-form-text" id="apply2">
			...
			<div class="layui-input-block">
				<textarea id="tReason2" placeholder="输入具体任务内容" class="layui-textarea"
					style="height: 300px">${task.TManagerview}</textarea>
			</div>
		</div>
			...
			<div class="layui-input-block">
				<textarea id="tReason3" placeholder="输入具体任务内容" class="layui-textarea"
					style="height: 300px">${task.TCheckerview}</textarea>
			</div>
		</div>
		<div class="layui-form-item">
			<div class="layui-input-block">
				<button id="okSure" hidden class="layui-btn" lay-submit lay-filter="okSure">确定</button>
				<button id="saveSure" hidden class="layui-btn" lay-submit lay-filter="saveSure">保存</button>
			</div>
		</div>
	</form>
<script type="text/javascript">
...
if(state=='start'){
	...
	$("#apply2").attr("hidden", "");
	$("#apply3").attr("hidden", "");
}else if(state=='apply'||state=='no apply'){
	$("#apply2").attr("hidden", "");
	$("#apply3").attr("hidden", "");
}else if(state=='agree'||state=='no agree'){
	$("#apply3").attr("hidden", "");
}else if(state=='save'){
	$("#okSure").removeAttr("hidden", "");
	$("#saveSure").removeAttr("hidden", "");
	$("#apply1").attr("hidden", "");
	$("#apply2").attr("hidden", "");
	$("#apply3").attr("hidden", "");
	$("#applying").attr("hidden", "");
}
	...
	<script type="text/javascript">
		layui.use('form', function() {
			...
			form.on('submit(agree)', function(data) {
				...
				wareView.act = 'agree';
				$.ajax({
					url : './adminWareInTask/taskOperate.do',
					type : 'POST',
					dateType : 'json',
			...
			form.on('submit(saveSure)', function(data) {
				var task = $("#taskForm").serializeArray();
				$.ajax({
