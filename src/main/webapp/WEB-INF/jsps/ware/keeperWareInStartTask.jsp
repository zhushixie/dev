...
<body>
	...
	<div style="text-align:center;font-size:30px;padding-top:20px;float:left">我的采购</div>
...
<script id="toolbarDemo" type="text/html">
  <div class="layui-btn-container">
    ...
    <button class="layui-btn layui-btn-sm" lay-event="isAll">任务取消</button>
	<button class="layui-btn layui-btn-sm" lay-event="isAll">执行采购</button>
	<button class="layui-btn layui-btn-sm" lay-event="isAll">采购完成</button>
  </div>
</script>
	<script>
layui.use('table', function(){
  ...
  table.render({
    elem: '#task'
    ,url:'./keeperWareInStartTask/list'
   	,toolbar: '#toolbarDemo'
  ...
  ,cols: [[
           ...
           ,{field:'tcreatetime', width:100, title: '发布时间', sort: true}
  ...
  table.on('tool(task)', function(obj){
	    ...
	    if(obj.event === 'purchase'){
	      if(data.tstate!=='agree'){
		      ...
		      if(data.tstate=='no agree' || data.tstate=='no apply'){
		    	  layer.msg('项目没有通过审批');
		      }else if(data.tstate=='no finish' ){
		    	  layer.msg('项目验收失败，重新验收');
		      }else{
		    	  layer.msg('项目正在审批中，无法撤回');
		    	  return;
		      }
