...
<form class="form-horizontal" role="form" method="post"
			...
			<div class="modal-dialog">
				<div class="modal-content">
					<div class="modal-header layui-bg-green">
						...
						<h4 class="modal-title" style="text-align: center">添加设备信息</h4>
					</div>
					<div class="modal-body" style="margin-left: 30px;">
						<input name="id" id="id" hidden="hidden" />
						<div class="form-group form-inline"
							style="float: left; margin-right: 10px">
							<label>设备名称：</label> <input type="text" name="name"
								class="form-control" id="name" />
						</div>
						<div class="form-group form-inline">
							<label>设备编码：</label> <input type="text" name="phone"
								class="form-control" id="phone" />
						</div>
						<div class="form-group form-inline"
							style="float: left; margin-right: 10px">
							<label>设备类型：</label> <input type="text" name="sex"
								class="form-control" id="sex" />
						</div>
						<div class="form-group form-inline">
							<label>购置日期：</label> <input type="text" name="addr"
								class="form-control" id="addr" />
						</div>
						<div class="form-group form-inline">
							<label>生产厂家：</label> <input type="text" class="layui-input"
								name="born" style="width: 60%; display: inline-block"
								class="form-control" id="born_add" />
						</div>
												<div class="form-group form-inline">
							<label>使用部门：</label> <input type="text" class="layui-input"
								name="born" style="width: 60%; display: inline-block"
								class="form-control" id="born_add" />
						</div>
																		<div class="form-group form-inline">
							<label>安装车间：</label> <input type="text" class="layui-input"
								name="born" style="width: 60%; display: inline-block"
								class="form-control" id="born_add" />
						</div>
						<div class="form-group form-inline">
							<label>维保到期：</label> <input type="text" class="layui-input"
								name="born" style="width: 60%; display: inline-block"
								class="form-control" id="born_add" />
						</div>
						<div class="form-group form-inline">
							<label>售后电话：</label> <input type="text" class="layui-input"
								name="born" style="width: 60%; display: inline-block"
								class="form-control" id="born_add" />
						</div>
						<div class="form-group form-inline">
							<label>售后联系人：</label> <input type="text" class="layui-input"
								name="born" style="width: 60%; display: inline-block"
								class="form-control" id="born_add" />
						</div>
					</div>
			...
			<div class="modal-dialog">
				<div class="modal-content">
					<div class="modal-header layui-bg-green">
						<button type="button" class="close" data-dismiss="modal"
							aria-hidden="true">&times;</button>
						<h4 class="modal-title" style="text-align: center">更改设备信息</h4>
					</div>
					<div class="modal-body" style="margin-left: 30px;">
						<input name="id" id="id" hidden="hidden" />
						<div class="form-group form-inline"
	...
	<div class="layui-layout layui-layout-admin">
		<div class="layui-header">
			<a href="./main.html">
				<div class="layui-logo">设备信息管理</div>
			</a>
		...
		<div class="layui-bg-green" style="padding-top:10px;padding-bottom:10px">
					...
					<div class="col-md-4 col-sm-8 col-xs-8">
						<button type="button" class="btn btn-success btn-sm" onclick="createDevInfo();"><i class="glyphicon glyphicon-asterisk"></i> 创建设备</button>
						<button type="button" class="btn btn-warning btn-sm" onclick="renameDevInfo();"><i class="glyphicon glyphicon-pencil"></i> 修改设备信息</button>
						<button type="button" class="btn btn-danger btn-sm" onclick="deleteDevInfo();"><i class="glyphicon glyphicon-remove"></i> 删除设备</button>
					</div>
								...
								function(e, data) {
									...
									if(!(data.node!==undefined)||!(data.node.id!==undefined)||data.node.parent=="#"){
										...
										if ($(".layui-tab-title li[lay-id]").length <= 0) {
											//如果比零小，则直接打开新的tab项
											active.tabAdd("./devInfoDetail",data.node.id,r);
										} else {
											//否则判断该tab项是否以及存在
											...
											if (isData == false) {
												//标志为false 新增一个tab项
												active
														.tabAdd("./devInfoDetail",data.node.id,r);
											}
										}
						...
						.jstree(
								{
									"core" : {
										...
										'data' : {
											'url' : function(node) {
												return node.id === '#' ? './devInfoMana/allRoots.do'
														: './devInfoMana/childrenByRoot.do';
											},
											'data' : function(node) {
												return {
													'id' : node.id
