...
<body>
			<div class="layui-bg-black"
				...
				<div class="layui-logo" style="margin-top: 20px;font-size:30px;padding-top:10px">工业设备管理</div>
				<div class="layui-btn-container layui-layout-right"
					style="margin-top: 20px; margin-right: 40px">
					<button class="layui-btn" id="addDev" onclick="addDev()">添加设备</button>
				</div>
			</div>
...
function addDev(){
    layui.use('layer', function(){
  	  ...
  	  layer.open({
  		  ...
  		  area: ['100%', '100%'],
  		  title:['添加设备','background:0x889600'],
  		  fixed: false, //不固定
  		  maxmin: true,
  		  resize:true,
  		scrolling:'yes',
  		  content: './devTypeAddDev.do?type=industry'
  		});
  	});
}
layui.use('table', function(){
  ...
  table.render({
    ...
    ,cols: [[
      ...
      ,{field:'tmodelid', width:100, title: '模型文件',sort: true}
		    ...
		    layui.use('layer', function(){
			    ...
			    layui.use('layer', function(){
			    	  ...
			    	  layer.open({
			    		  ...
			    		  skin: 'layui-layer-lan',
			    		  area: ['100%', '100%'],
			    		  title:['编辑设备','background:0x889600'],
			    		  fixed: false, //不固定
			    		  maxmin: true,
			    		  resize:true,
			    		  content: './devTypeEditDev.do?id='+data.id
			    		});
			    	});
		    }
}); 
